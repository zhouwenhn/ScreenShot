...
package com.chowen.screenshot.screenshot.cropper.widget;
...
import cn.ninegame.gamemanagerhd.share.screenshot.cropper.handle.Handle;
import cn.ninegame.gamemanagerhd.share.screenshot.util.AspectRatioUtil;
import cn.ninegame.gamemanagerhd.share.screenshot.util.DensityUtil;
import cn.ninegame.gamemanagerhd.share.screenshot.util.Edge;
import cn.ninegame.gamemanagerhd.share.screenshot.util.HandleUtil;
import cn.ninegame.gamemanagerhd.share.screenshot.util.PaintUtil;
...
public class CropOverlayView extends View {
	...
	private void drawCorners(Canvas canvas) {
		...
		int mPx = DensityUtil.dip2px(mContext, 10);

		Drawable mCropPointDrawable = mContext.getResources().getDrawable(
				R.drawable.btn_hold);
		// Top left
		mCropPointDrawable.setBounds(left - mPx, top - mPx, left
				+ mCropPointDrawable.getIntrinsicWidth() - mPx, top
				+ mCropPointDrawable.getIntrinsicHeight() - mPx);
		mCropPointDrawable.draw(canvas);

		// Top right
		mCropPointDrawable.setBounds(
				right - mCropPointDrawable.getIntrinsicWidth() + mPx,
				top - mPx, right + mPx, top + mCropPointDrawable.getIntrinsicHeight() - mPx);
		mCropPointDrawable.draw(canvas);

		// Bottom left
		mCropPointDrawable.setBounds(left - mPx,
				bottom - mCropPointDrawable.getIntrinsicHeight() + mPx, left
						+ mCropPointDrawable.getIntrinsicWidth() - mPx, bottom + mPx);
		mCropPointDrawable.draw(canvas);

		// Bottom right
		mCropPointDrawable.setBounds(
				right - mCropPointDrawable.getIntrinsicWidth() + mPx, bottom
						- mCropPointDrawable.getIntrinsicHeight() + mPx, right
						+ mPx, bottom + mPx);
		mCropPointDrawable.draw(canvas);
	...
	//zoom in  
   public void  reCropWindowSize(Rect bitmapRect) {
